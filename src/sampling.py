...
def mnist_noniid_unequal(dataset, num_users):
    ...
    random_shard_size = np.random.randint(min_shard,
                                          max_shard + 1,
                                          size=num_users)
    random_shard_size = np.around(random_shard_size / sum(random_shard_size) *
                                  num_shards)
...
def cifar_noniid(dataset, num_users):
    ...
    idxs = np.arange(num_shards * num_imgs)
    # labels = dataset.train_labels.numpy()
    labels = np.array(dataset.targets)  #train_labels)version issue pytorch.
...
def cifar_noniid_unequal(dataset, num_users):
    ...
    dict_users = {i: np.array([]) for i in range(num_users)}
    idxs = np.arange(num_shards * num_imgs)
    labels = np.array(dataset.targets)  #dataset.train_labels.numpy()
